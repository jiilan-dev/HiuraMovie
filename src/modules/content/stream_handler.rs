...
/// Stream episode content with support for Range requests
/// Proxies the stream from S3/MinIO to the client efficiently
#[utoipa::path(
    get,
...
pub async fn stream_episode(
    ...
    let episode = match crate::modules::content::repository::ContentRepository::get_episode_by_id(&state.db, id).await {
        Ok(Some(e)) => e,
        Ok(None) => return StatusCode::NOT_FOUND.into_response(),
        Err(e) => {
            tracing::error!("Database Error: {}", e);
